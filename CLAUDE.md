...
- **访客记录**（「除了我还有谁看过站点」）：游客入口是**所有人共用的一个 guest 账号**，应用日志分不出个体 —— `src/server/visit-log.ts` 按每次登录事件落 `visit_events` 表（IP / `ip_prefix` / 属地 / UA / `device` / `is_bot` / referer / `src` 渠道 / `landing`），在 `/admin` 页「🕵️ 访客记录」聚合查看（`GET /api/admin/visits`，仅管理员；`POST /api/admin/visits/resolve` 后台补属地）。`record()` 是 fire-and-forget（DB 抖动只丢一条观测记录、绝不影响登录）；属地走 ip-api.com（**免费版仅 HTTP**，返回中文），解析成功按 IP 回写历史行 + 进程内缓存，内网/`unknown` 地址不外查（直接标 `local`）。`TRAFFIC_SELF_IPS` 标出自己（前端「隐藏我自己」），`TRAFFIC_GEO_ENABLED=false` 关外查，`TRAFFIC_GEO_ENDPOINT` 可换服务商。迁移 `004_create_visit_events.sql`（幂等：`migrate.ts` 每次启动重跑全部 .sql）
- **反代后必须设 `trust proxy`**（`app.set('trust proxy', N)`，env `H5_TRUST_PROXY_HOPS` 默认 1）：不设则 `req.ip` 恒为**代理容器**地址 —— ① 访客记录里全是 172.x，「谁来过」无从分辨；② 登录防爆破按 IP 计数会把全网访客当同一人（任一人失败 5 次即锁住所有人，条件式校验码也对所有人一起亮）。取 1 而非 `true`：只信任一跳时 Express 取 XFF **最右**一项 —— 那正是 Caddy 写入的真实客户端地址（最左一项可被客户端伪造）。域名前还挂 CDN 时按实际层数加，直连部署设 0
